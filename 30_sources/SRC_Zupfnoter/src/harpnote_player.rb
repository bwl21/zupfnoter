    ...
    class HarpnotePlayer
      ...
      def play_notes(the_notes)
        ...
        unless the_notes.empty?
          ...
          stop_time = (lastnote[:delay] - firstnote[:delay] + $conf.get('active.SHORTEST_NOTE') * @duration_timefactor) * 1000 # todo factor out the literals
      ...
      def load_song(music)
        ...
        @beat_timefactor = 1/(tf * $conf.get('active.BEAT_PER_DURATION')) # convert music beat to musicaljs delay
