...
Music.prototype.outVoices = function (b, a) {
    ...
    for (k in this.vnums)if (0 != this.cnt.getv("note", k)) {
        ...
        for (l = 0; n.length;) {
            ...
            n.splice(0, m);
            c = sortitems(p, 1);
            for (d in c)e = c[d], f = e[0], e = e[1], abcOut.add("w: " +
...
ABCoutput.prototype.mkHeader = function (b, a, c) {
    ...
    f = [];
    for (m in this.cmpL)e = this.cmpL[m], f[e] = (f[e] || 0) + 1;
    f = sortitems(f);
    f = f[f.length - 1][0];
...
function outVoice(b, a, c, d, f) {
    for (d = 0; d < b.length;)c = b[d], c instanceof Note && c.fact && (c = insTup(d, b, [1, 1]), d = c[0]), d += 1;
    d = [];
...
Parser.prototype.ntAbc = function (b, a, c, d) {
    var f = {"double-flat": -2, "flat-flat": -2, flat: -1, natural: 0, sharp: 1, "sharp-sharp": 2, "double-sharp": 2};
...
Parser.prototype.doPartList = function (b) {
    ...
    for (a = 0; a < f.length; ++a) {
        ...
        for (c = 0; c < g.length; ++c) {
            ...
            h = [];
            for (d in k)m = k[d], h.push(l.find(m).text() || this.midDflt[d]);
            k = h[3];
...
Parser.prototype.mkTitle = function (b) {
    ...
    e = function (b) {
        ...
        var k = [], h, l;
        for (l in e)h = e[l], 6 > b && (h && -1 < a.indexOf(h) || h && -1 < c.indexOf(h)) || 5 > b && (h && -1 < d.indexOf(h) || h && -1 < f.indexOf(h)) || 4 > b && (a && -1 < h.indexOf(a) || c && -1 < h.indexOf(c)) || 3 > b && (d.some(g) || f.some(g)) || 2 > b && /^[\d\W]*$/.test(h) || k.push(h);
        0 == b && a + c && (k = "");
        return k
    }(this.ctf);
    a && (a = "T:" + a + "\n");
...
Parser.prototype.addStaffMap = function (b) {
    ...
    for (a in l) {
        ...
        d = this.stfMap[f];
        e = [];
        for (a in d)c = d[a], c in b && e.push(b[c]);
        if (e.length)for (a in g.push(e.sort()), d = f in this.clefMap ? this.clefMap[f] : "treble", e)c = e[a], abcOut.clefs[c] = d
    }
    this.gStfMap.push(g)
};
